...
1. 使用 [🧮 在线计算器](https://openquad-calc.ea4ipw.es) 或公式，以 Vf = 1.0 计算所有元件的尺寸（`驱动元周长 = 1005 / f(MHz) × 304.8 mm`）。
2. 按这些尺寸组装完整的天线。
3. 用 VNA 测量整个天线的谐振频率。
4. 将测得的频率（与目标频率）输入 [🧮 计算器](https://openquad-calc.ea4ipw.es)，得到真实的 Vf 及修正后的尺寸。
