...
1. [🧮 オンライン計算機](https://openquad-calc.ea4ipw.es) または公式を使って、Vf = 1.0 ですべてのエレメントの寸法を計算します（`ドリブンの周長 = 1005 / f(MHz) × 304.8 mm`）。
2. その寸法でアンテナ全体を組み立てます。
3. VNA で組み立てたアンテナの共振周波数を測定します。
4. 測定した周波数（と目標周波数）を [🧮 計算機](https://openquad-calc.ea4ipw.es) に入力し、実際の Vf と補正後の寸法を得ます。
